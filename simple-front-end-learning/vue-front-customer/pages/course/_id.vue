            ...
            <div>
              ...
              <span class="ml10 vam">
                ...
                <a class="c-fff vam" title="收藏" href="#">收藏</a>
                        ...
                        <section class="c-attr-mt" v-if="this.isBuy||Number(courseWebVo.price) === 0">
                            <a  href="javascript:void(0)" title="立即观看" class="comm-btn c-btn-3">立即观看</a>
                        </section>
                        <section class="c-attr-mt" v-else>
                            <a @click="createOrder()" href="javascript:void(0)" title="立即购买" class="comm-btn c-btn-3">立即购买</a>
                        </section>
                    </section>
                </aside>
                                        ...
                                        <div class="lh-menu-wrap">
                                            <menu id="lh-menu" class="lh-menu mt10 mr10">
                                                <ul>
                                                    <!-- 文件目录  -->
                                                    <!-- 此处有bug 大纲显示后马上消失 -->
                                                    <li class="lh-menu-stair" v-for="chapter in chapterVideoList"
                                                        :key="chapter.id">
                            ...
                            <section class="stud-act-list">
                                <ul style="height: auto;">
                                    <li>
                                        <div class="u-face">
                                            <a href="#">
                                                <img :src="courseWebVo.avatar" width="50"
                                                     height="50" alt>
                                            </a>
                                        </div>
...
<script>
	...
	export default {
		...
		methods: {
			// getCourseInfo() {
			// 	courseApi.getCourseInfo(this.id).then(response => {

			// 		if (response.data.data.course !== null) {
			...
			initComment() {
				commentApi.getCommentList(this.current, this.size, this.courseId).then(response => {
			...
			addComment() {
				...
				commentApi.addComment(this.comment).then(response => {
					if (response.data.success) {
						...
						//提示
						this.$message.error('您暂未登录,无法发表评论');
						this.comment.content = ''
                    }
				})
			},
			gotoPage(current) {
				commentApi.getCommentList(current, this.size, this.courseId).then(response => {
					this.data = response.data.data
				})
			},
			createOrder(){
